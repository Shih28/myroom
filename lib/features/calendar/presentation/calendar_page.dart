...
String _hm(DateTime dt) => '${fmt2(dt.hour)}:${fmt2(dt.minute)}';
...
class CalendarPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return StreamProvider<List<CalendarEvent>>(
...
class _CalendarBodyState extends State<_CalendarBody> {
  ...
  // All events (allDay or not) span their full date range.
  ...
  Widget _buildViewForDate(DateTime date, List<CalendarEvent> events) {
    switch (_view) {
      case CalendarView.month:
        return _MonthView(
          year: date.year,
...
class _WeekViewState extends State<_WeekView> {
  ...
  void initState() {
    ...
    _scrollCtrl = ScrollController(
      initialScrollOffset: (scrollHour * kHourH - 60.0).clamp(
        0.0,
        double.infinity,
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final allDayEvts = widget.events.where((e) => e.isAllDay).toList();
    final timedEvts = widget.events.where((e) => !e.isAllDay).toList();
                              ...
                              child: GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    border: e.isAllDay
                                        ? null
                                        : Border.all(color: e.color, width: 1),
                                  ),
                                  child: Text(
                                    e.title,
                                    style: const TextStyle(
                    ...
                    child: Stack(
                      children: List.generate(
                        ...
                        (h) => Positioned(
                          ...
                          top: h == 0 ? 2.0 : h * kHourH.toDouble() - 7,
                          child: SizedBox(
                            width: 42,
                            child: Text(
                              '${fmt2(h)}:00',
                    ...
                    return Expanded(
                      child: GestureDetector(
                        ...
                        onTapDown: (details) {
                          ...
                          final minute = ((tapH - hour) * 60).round().clamp(
                            0,
                            59,
                          );
                        ...
                        child: Stack(
                          ...
                          children: [
                            ...
                            ...dayTimed.map((e) {
                              ...
                              final dNext = dMid.add(const Duration(days: 1));
                              final dispStart = eStart.isBefore(dMid)
                                  ? dMid
                                  : eStart;
                              final dispEnd = eEnd.isAfter(dNext)
                                  ? dNext
                                  : eEnd;
                              final topFrac =
                                  dispStart.hour + dispStart.minute / 60.0;
                              final durH =
                                  dispEnd.difference(dispStart).inMinutes /
                                  60.0;
...
class _DayView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final allDay = events
        .where((e) => e.isAllDay && eventIsOnDay(e, date))
        .toList();
    final timed =
        events.where((e) => !e.isAllDay && eventIsOnDay(e, date)).toList()
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: GestureDetector(
            onTap: () => onAdd(DateTime(date.year, date.month, date.day, 9, 0)),
            child: Container(
              width: double.infinity,
...
class _AddEventModalState extends State<_AddEventModal> {
  ...
  Future<void> _pickStartTime() async {
    final t = await _show24hTimePicker(
      context,
      TimeOfDay.fromDateTime(_startDt),
    );
  ...
  Future<void> _pickEndDate() async {
    ...
    setState(
      () =>
          _endDt = DateTime(d.year, d.month, d.day, _endDt.hour, _endDt.minute),
    );
  }

  Future<void> _pickEndTime() async {
    final t = await _show24hTimePicker(context, TimeOfDay.fromDateTime(_endDt));
    if (t == null || !mounted) return;
    setState(
      () => _endDt = DateTime(
        _endDt.year,
  ...
  void _save() {
    ...
    final start = _allDay
        ...
        : _startDt;
    // New events default endTime = startTime + 1h when unset; when allDay,
    // span to 23:59 of the end day to mirror the demo's full-day behavior.
    DateTime end = _allDay
            ...
            Row(
              children: [
                ...
                if (!_allDay) ...[
                  ...
                  Expanded(
                    child: _DateTimeChip(
                      label: '${fmt2(_startDt.hour)}:${fmt2(_startDt.minute)}',
            ...
            Row(
              children: [
                ...
                if (!_allDay) ...[
                  ...
                  Expanded(
                    child: _DateTimeChip(
                      label: '${fmt2(_endDt.hour)}:${fmt2(_endDt.minute)}',
                      icon: LucideIcons.clock,
                      onTap: _pickEndTime,
                    ),
                  ),
...
class _EventDetailSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final durMin = en.difference(s).inMinutes;
        ...
        children: [
          ...
          if (!e.isAllDay)
            _DetailRow(
              icon: LucideIcons.clock,
              text: isMultiDay
                  ? '${s.year}年${s.month}月${s.day}日 ${_hm(s)} – ${en.year}年${en.month}月${en.day}日 ${_hm(en)}'
                  : '${s.year}年${s.month}月${s.day}日  ${_hm(s)} – ${_hm(en)}  （$durMin 分鐘）',
            )
          else
            _DetailRow(
              icon: LucideIcons.calendar,
              text:
                  '全天  ${s.year}年${s.month}月${s.day}日 – ${en.month}月${en.day}日',
            ),
          ...
          if (e.description != null && e.description!.isNotEmpty) ...[
            const SizedBox(height: 10),
            _DetailRow(icon: LucideIcons.textAlignStart, text: e.description!),
          ],
          const SizedBox(height: 24),
          GestureDetector(
